...
const gettMemberCartData = async () => {
  const res = await getMemberCartAPI()
  cartList.value = res.result
}
onShow(() => {
  if (store.profile) gettMemberCartData()
})
...
const onChangeCount = async (e: InputNumberBoxEvent) => {
  await putMemberCartSkuIdAPI(e.index, { count: e.value })
}
...
const onChangeSelectedAll = () => {
  const _isSelectedAll = !isSelectedAll.value // 全选状态取反
  cartList.value.forEach((item) => (item.selected = _isSelectedAll)) // 前端数据更新
  putMemberCartSelectedAPI({ selected: _isSelectedAll }) // 后端数据更新
}
...
const goToPayment = () => {
  ...
  // 跳转到结算页
  uni.navigateTo({ url: '/pagesOrder/pages/create/create' })
}
</script>
